...
struct NetworkService {
    ...
    private func handleResponse<T:Decodable>(result:Result<Data,Error>?,completion: (Result<T,Error>) -> Void) {
        ...
        case .success(let data):
            ...
            if let errror = response.error {
                completion(.failure(AppError.serverError(errror)))
                return
            }
   ...
   private func createRequest(route : Route,
        ...
        if let params = parameters {
            switch method {
            case .get:
                var urlCompoent = URLComponents(string: urlString)
                urlCompoent?.queryItems = params.map { URLQueryItem(name: $0, value: "\($1)") }
                urlRequest.url = urlCompoent?.url
